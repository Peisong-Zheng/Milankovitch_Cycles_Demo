...
深色星空主题的双联 Three.js 场景——左侧为地球轨道全景，右侧为同一颗地球的
特写（卡通地球、晨昏线、整颗星随岁差摆动），展示
...
- 右屏地球特写（离线生成的平面卡通纹理，海陆分色 + 海岸线 + 高纬冰盖）：
  - 两种光照模式（右下角开关）：
    - 锁定模式（默认）：阳光固定从左前方射入——代表一个固定的轨道位置；
      整颗地球（贴图、地轴、纬度圈为一体）在阳光下做逆行岁差摆动，
      地轴相对阳光在 ±ε 间摇摆，该位置的季节在北半球夏与冬之间缓慢翻转
      （起始时刻恰为"北半球夏至"光照），极昼极夜交替可见；
    - 季节模式：岁差冻结，阳光仰角随年内时间在 −2ε ~ 0 间摆动
      （β = δ − ε，â·ŝ = sinδ 严格成立），单独展示 obliquity 的季节效应；
  - 地轴倾角 ε 随 kyr 呼吸（obliquity）；
  - 相机正对 0° 经线（起始时刻大西洋/欧洲/非洲朝向观察者）；
  - 球面上只保留赤道与 65°N 亮环（"冰期关键纬线"）；
  - 读出板显示：北半球夏至相对近日点的真近点角（岁差最直观的指标：
    0° = 夏至逢近日点，180° = 逢远日点；与左屏轨道几何严格一致）、
    65°N 夏至日均日照量（W/m²，由 e、ε、ϖ 纯公式计算）及与今日的差值 Δ。
...
注意：地球在轨道上的相位与公转速度为视觉示意；场景中的偏心率夸张 ×3 以便观察，
HUD 与图表始终显示真实数值。
...
node tools/check_orbit_signs.mjs  # 轨道符号约定：公转CCW / 拱线顺行 / 赤道逆行 / 夏至位置
```
...
js/earthview.js   右屏地球特写（卡通纹理、锁定/季节双光照模式、整星岁差摆动、0°经线取景）
js/charts.js      Canvas 时间序列图（深色仪表盘配色）
js/player.js      播放引擎（SPEED_KYR_PER_SEC 调整速度）
js/main.js        入口：组装双视图 / 图表 / 播放器，持有共享年相位时钟
