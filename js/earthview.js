...
export class EarthView {
  constructor(canvas, data) {
    ...
    // Key light = Sun, from the left (fixed) — strong so the day side reads
    // bright; fill + ambient kept faint so the night side stays clearly dark
    this.sunLight = new THREE.DirectionalLight(0xfff7e0, 2.8);
  ...
  _buildEarth() {
    ...
    // NASA Blue Marble texture (vendored from the three.js r160 examples);
    // falls back to the plain cartoon blue while loading or if missing
    ...
    this.globe = new THREE.Mesh(new THREE.SphereGeometry(R, 64, 48), this.globeMat);
    this.globe.rotation.y = TEX_LON0; // lon 0 faces the camera at γ = 0
